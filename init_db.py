...
Fixes missing columns and initializes all required tables
"""
...
def init_database():
    ...
    cursor.execute("DROP TABLE IF EXISTS markov_zip_states")
    cursor.execute("DROP TABLE IF EXISTS leads")
    cursor.execute("DROP TABLE IF EXISTS storms")
    cursor.execute("DROP TABLE IF EXISTS routes")
    cursor.execute("DROP TABLE IF EXISTS jobs")
    cursor.execute("DROP TABLE IF EXISTS earth2_impact_zones")
    cursor.execute("DROP TABLE IF EXISTS voice_calls")
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_leads_zip ON leads(zip_code)")
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_leads_score ON leads(lead_score DESC)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_markov_tenant_storm ON markov_zip_states(tenant_id, storm_id)"
    )
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_zones_storm ON earth2_impact_zones(storm_id)"
    )
    ...
    cursor.execute("DELETE FROM storms")
    cursor.execute("DELETE FROM leads")
    cursor.execute("DELETE FROM earth2_impact_zones")
    cursor.execute("DELETE FROM markov_zip_states")
    ...
    for zip_code, hail, wind, damage, roofs, value, risk in sample_zips:
        ...
        """,
            (
                ...
                value,
                risk,
                32.7767 + (hash(zip_code) % 100) / 1000.0,
                -96.7970 + (hash(zip_code) % 100) / 1000.0,
            ),
        )
    ...
    cursor.execute("DELETE FROM markov_zip_states")
    markov_states = [
        ("demo-tenant", "demo-storm", "75201", "impact", 0.92, 71736000),
        ("demo-tenant", "demo-storm", "75204", "impact", 0.88, 55228000),
        ("demo-tenant", "demo-storm", "75024", "impact", 0.91, 54252000),
        ("demo-tenant", "demo-storm", "75230", "impact", 0.93, 68288000),
        ("demo-tenant", "demo-storm", "75240", "recovery", 0.85, 53625000),
        ("demo-tenant", "demo-storm", "75034", "impact", 0.87, 46704000),
        ("demo-tenant", "demo-storm", "75205", "impact", 0.82, 40968000),
        ("demo-tenant", "demo-storm", "75225", "impact", 0.86, 47714000),
        ("demo-tenant", "demo-storm", "75001", "recovery", 0.78, 22776000),
        ("demo-tenant", "demo-storm", "75002", "impact", 0.89, 34105000),
        ("demo-tenant", "demo-storm", "75252", "impact", 0.84, 42724000),
        ("demo-tenant", "demo-storm", "75035", "recovery", 0.76, 26913000),
    ]

    for tenant, storm, zip_code, state, prob, tam in markov_states:
        cursor.execute(
            """
            INSERT INTO markov_zip_states (tenant_id, storm_id, zip_code, current_state, prob_to_recovery, estimated_tam_usd, state_entered_at)
            VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        """,
            (tenant, storm, zip_code, state, prob, tam),
        )

    cursor.execute("DELETE FROM routes")
    route_data = [
        ("route-001", "North Dallas Premium", "75201", 145, 67, 18900000),
        ("route-002", "Plano East Cluster", "75024", 98, 45, 12400000),
        ("route-003", "Frisco Central", "75034", 124, 58, 16750000),
    ]

    for route_id, name, zip_code, doors, completed, value in route_data:
        cursor.execute(
            """
            INSERT INTO routes (route_id, storm_id, tenant_id, name, zip_code, property_count, total_doors, completed_doors, estimated_value, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (
                route_id,
                ...
                name,
                zip_code,
                doors,  # property_count equals total_doors
                doors,
                completed,
                value,
                "in_progress" if completed > 0 else "pending",
            ),
        )

    cursor.execute("DELETE FROM jobs")
    job_data = [
        ("job-001", "lead-001", "route-001", "123 Main St", "Completed exterior inspection", "completed", 18500, "hail"),
        ("job-002", "lead-003", "route-001", "456 Oak Ave", "In-progress roof assessment", "in_progress", 0, "hail"),
        ("job-003", "lead-005", "route-002", "789 Pine Rd", "Scheduled for tomorrow", "pending", 0, "hail"),
        ("job-004", "lead-007", "route-002", "321 Elm Blvd", "Waiting for permit", "pending", 0, "wind"),
    ]

    from datetime import datetime, timedelta

    for job_id, lead_id, route_id, address, notes, status, amount, dmg in job_data:
        completed = (
        ...
        cursor.execute(
            """
            INSERT INTO jobs (job_id, lead_id, route_id, address, notes, status, claim_amount, damage_type, completed_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """,
            (job_id, lead_id, route_id, address, notes, status, amount, dmg, completed),
        )
    ...
    cursor.execute("""
        ...
        VALUES
            ...
            ('demo-tenant', 'demo-storm', 'action-003', 'build_routes', 'proposed', 0.78, 'Build 2 optimized routes for 1,500 doors', '{}', CURRENT_TIMESTAMP),
    ...
    print(
        f"Markov States: {len(markov_states)} | Routes: {len(route_data)} | Jobs: {len(job_data)}"
    )
